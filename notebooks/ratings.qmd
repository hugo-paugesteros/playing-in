...
plt.savefig("../reports/figures/ratings.svg")
```
...
g = sns.FacetGrid(
    data=(
        df.assign(
            scope_pretty=lambda x: x["scope"].str.title(),
            criterion_pretty=lambda x: x["criterion"].map(CRITERION_MAP),
            violin_pretty=lambda x: x["violin"].map(VIOLIN_MAP),
        )
    ),
    row="violin_pretty",
    col="criterion_pretty",
    margin_titles=True,
    height=WIDTH / RATIO,
    aspect=RATIO,
)
...
g.set_axis_labels("Violinist", "Rating (0-10)")
# plt.tight_layout()
for ax in g.axes[:, -1]:
    for txt in ax.texts:
