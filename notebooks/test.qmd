...
```{python}
VIOLINS = {"A": "Klimke", "B": "Levaggi", "C": "Stoppani"}
with open("../data/raw/test.json", "r") as file:
    test = json.load(file)
    for i, (stimuli1, stimuli2) in enumerate(test):
        v1 = VIOLINS[stimuli1["violin"]]
        v2 = VIOLINS[stimuli2["violin"]]
        print(
            f"[{i+1}], [{v1}], [{stimuli1['player']}], [{stimuli1['session']}], [{v2}], [{stimuli2['player']}], [{stimuli2['session']}],"
        )
```
...
labels_mapping = {
    "SMD": "Test Player",
    "Norimi": "Control Player",
}
order_list = list(labels_mapping.keys())
...
plt.savefig("../reports/figures/tests.svg")
